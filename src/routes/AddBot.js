...
import { Form, Container, Message, TextArea, Segment, Label, Divider, Dropdown, Input } from 'semantic-ui-react'
...
class SubmitBot extends Component {
  state = { id: '', lib: '', prefix: '', intro: '', desc: '', category: [], website: '', git: '', url: '', discord: '', data: { state: 0, data: { }} }
  ...
  handleSubmit = async () => {
   ...
   console.log(this.state)
    if(this.state.id && this.state.lib && this.state.intro && this.state.desc && this.state.category){ 
        data = await this.sendSumbit(this.state)
    } else {
        console.log('GG')
        this.setState({ data: { state: 2, data: { message: '필수 입력칸을 전부 작성해주세요!'} }})
    }
  ...
  render() {
    const { id, prefix, lib, intro, desc, category, website, git, url, discord, data } = this.state
    ...
    <h1>로그인 해주세요!</h1>
      ...
      <Container>
        ...
        <br/>
            <label>카테고리 (*)</label>
            <Dropdown name='category' placeholder='해당하는 카테고리들을 선택해주세요!!' fluid multiple selection options={options} onChange={this.handleChange}  />
            </div>
            <Form.Input
        ...
        <pre>{JSON.stringify({ id, prefix, lib, intro, desc, category, website, git, url, discord }, null, 2)}</pre>
...
const options = [{"text":"관리","value":"관리","key":"관리"},{"text":"뮤직","value":"뮤직","key":"뮤직"},{"text":"전적","value":"전적","key":"전적"},{"text":"웹 대시보드","value":"웹 대시보드","key":"웹 대시보드"},{"text":"로깅","value":"로깅","key":"로깅"},{"text":"도박","value":"도박","key":"도박"},{"text":"게임","value":"게임","key":"게임"},{"text":"밈","value":"밈","key":"밈"},{"text":"레벨링","value":"레벨링","key":"레벨링"},{"text":"유틸리티","value":"유틸리티","key":"유틸리티"},{"text":"번역","value":"번역","key":"번역"},{"text":"대화","value":"대화","key":"대화"},{"text":"NSFW","value":"NSFW","key":"NSFW"},{"text":"검색","value":"검색","key":"검색"}]